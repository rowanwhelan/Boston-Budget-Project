...
class TestMainWorkflow(unittest.TestCase):
    ...
    def test_data_preprocessing(self, mock_read_csv):
        ...
        mock_read_csv.return_value = pd.DataFrame({
            'Variable': ['Education', 'Health', 'Transport'],
            'Year': [2022, 2022, 2022],
            'MA: Boston': [100, None, 150],
            'NY: New York': [None, 200, None]
        })

        # Step 1: Preprocess the data
        preprocessed_data = preprocess_data(mock_read_csv.return_value)
        # Step 2: Perform assertions to ensure the function works as expected
        
        # Assert that the processed data contains the correct columns
        self.assertIn('Variable', preprocessed_data.columns)
        self.assertIn('Year', preprocessed_data.columns)
        self.assertIn('City', preprocessed_data.columns)
        self.assertIn('Budget', preprocessed_data.columns)

        # Assert that 'Budget' column is numeric
        self.assertTrue(pd.api.types.is_numeric_dtype(preprocessed_data['Budget']))

        # Assert that 'Year' column is numeric
        self.assertTrue(pd.api.types.is_numeric_dtype(preprocessed_data['Year']))

        # Additional checks: if we have multiple cities, make sure the data is correctly shaped
        self.assertEqual(preprocessed_data.shape[0], 6)  # Three rows in the original mock data

        # Check if the Budget values remain as expected after preprocessing (no commas, etc.)
        self.assertEqual(preprocessed_data['Budget'].iloc[0], 100)
        self.assertEqual(preprocessed_data['Budget'].iloc[5], 200)
        self.assertEqual(preprocessed_data['Budget'].iloc[2], 150)

    @patch('sklearn.model_selection.train_test_split')
    @patch('sklearn.ensemble.GradientBoostingRegressor')
    def test_train_gbm(self, mock_gbm, mock_train_test_split):
        # Mock the data split and model training
        mock_train_test_split.return_value = (pd.DataFrame({
            ...
            'Budget': [100, 200],
            'Year': [2022, 2022]
        }), pd.Series([100, 200]), pd.Series([150, 250]))
        
        mock_gbm.return_value = MagicMock()
        mock_gbm.return_value.fit.return_value = mock_gbm

        # Train the model
        X_train, X_test, y_train, y_test = prepare_data_for_gbm_category(pd.DataFrame({
            'City': ['Boston', 'New York'],
            'Variable': ['Education', 'Health'],
            'Budget': [100, 200],
            'Year': [2022, 2022]
        ...
        # Assert that the model's fit method was called
        model.fit.assert_called_once()
        
    @patch('sklearn.ensemble.GradientBoostingRegressor.predict')
    def test_generate_future_predictions(self, mock_predict):
        # Mocking the predict method of the model
        mock_predict.return_value = np.array([300, 400])
        
        # Simulate calling generate_future_predictions
        data = pd.DataFrame({'City': ['Boston', 'New York'], 'Budget': [100, 200], 'Year': [2022, 2022]})
        gbm_model_boston = MagicMock()
        gbm_model_boston.predict.return_value = np.array([300, 400])  # Mock predictions
        
        complete_data = generate_future_predictions(data, gbm_model_boston, "MA: Boston", start_year=2022, end_year=2025)  # Replace with your actual function

        # Assert that predictions are generated and added to complete_data
        self.assertTrue('Predicted' in complete_data.columns)
        self.assertEqual(complete_data.shape[0], 4)  # Expect 4 rows with predictions for 2022-2025

    @patch('budget_modelling.visualize_boston_predictions')
    def test_main_workflow(self, mock_visualize):
        # Mock the actual visualization functions
        mock_visualize.return_value = None

        # Mock all other functions
        with patch('pandas.read_csv') as mock_read_csv:
            with patch('budget_modelling.train_gbm') as mock_train_gbm:
                with patch('budget_modelling.generate_future_predictions') as mock_generate_predictions:
                    ...
                    mock_train_gbm.return_value = MagicMock()
                    mock_generate_predictions.return_value = pd.DataFrame({
                        'City': ['Boston', 'New York'],
                        'Variable': ['Education', 'Health'],
                        'Year': [2022, 2023],
                        'Budget': [100, 200],  # Renamed 'Value' to 'Budget'
                        'Predicted': [300, 400]
                    })
